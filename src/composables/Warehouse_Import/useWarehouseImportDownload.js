...
export function useWarehouseImportDownload() {
            ...
            try {
                ...
                if (response && response.data.status === 'success' && response.data.url) {
                    ...
                    if (projectCode && ticket_id) {
                        downloadFileName.value = `PhieuNhapKho_${projectCode}-${ticket_id}`;
                    } else if (ticket_id) {
                        downloadFileName.value = `PhieuNhapKho_${ticket_id}`;
                    } else if (projectCode) {
                        downloadFileName.value = `PhieuNhapKho_${projectCode}`;
                    } else {
                        downloadFileName.value = `PhieuNhapKho_${new Date().toISOString().slice(0,10)}`;
                    }
